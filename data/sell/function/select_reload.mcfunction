...
execute positioned ~ ~1.2 ~ as @e[type=item,distance=..2] if data entity @s Item{id:"minecraft:emerald", components: {"minecraft:custom_data": {menu: "sell"}}} run kill @s
execute positioned ~ ~1.2 ~ as @e[type=item,distance=..2] if data entity @s Item{id:"minecraft:barrier", components: {"minecraft:custom_data": {menu: "sell"}}} run kill @s
...
item replace entity @s player.crafting.3 with minecraft:golden_apple[custom_data={menu:"sell",panel:"3"},item_name='{"text":"売却","italic":false,"color": "#efef25"}',lore=['{"italic":false,"color":"dark_gray","text":"クリックしてアイテムを売ります。"}']]
